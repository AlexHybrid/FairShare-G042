...
def index():
    """
    Serves the main frontend page (index.html/Dashboard.html) when a user visits the root URL.
    """
...
def static_files(path):
    """
    Serves all other frontend static files (CSS, JS, images, etc.) required by the browser.
    """
...
def get_expenses():
    """
    Fetches all recorded expenses from the SQLite database and returns them as a JSON list.
    Used by the frontend to populate the dashboard table and donut chart.
    """
...
def add_expense():
    """
    Receives data from the frontend's "Add Expense" form and inserts a new expense record into the database.
    """
    data = request.json
    date           = data.get('date')
    room           = data.get('room')
...
def update_expense_status(id):
    """
    Updates the status of a specific expense (e.g., changing it from 'Pending' to 'Paid').
    """
    data = request.json
...
def upload_receipt():
    """
    Handles file uploads for receipts (images/PDFs) and securely saves them to the 'uploads/' directory.
    """
...
def download_excel():
    """
    Gathers all expense data, formats it into a spreadsheet using openpyxl, and sends it as an .xlsx download.
    """
    wb = Workbook()
    sheet = wb.active
...
def download_pdf():
    """
    Gathers all expense data, formats it into a table using reportlab, and sends it as a .pdf download.
    """
    conn = get_db_connection()
    cursor = conn.cursor()
